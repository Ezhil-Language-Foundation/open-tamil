...
def spell():
    ...
    if u"word" in request.form:
        ...
        if ok:
            ...
            data_suggs2 = u"<br/>\n".join(
                [
                    u"<option>%d %s</option>" % (itr + 1, s_word)
                    for itr, s_word in enumerate(data_suggs.values()[0])
                ]
            )
            HTML = (
                "<B style='color:red;'>Not OK!</B><br /><select>"
                + data_suggs2
                + "\n</select><br/>"
            )
...
def spell_get_only():
    if u"word" in request.args:
        ...
        return (
            "<B style='color:red;'>Not OK!</B><br />" + data_suggs2 + "\n<br/>" + suggs
        )
